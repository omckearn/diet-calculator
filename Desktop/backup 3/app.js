/*
Elementor parent page snippet (HTML widget):

<script>
  (function () {
    const iframeEl = document.querySelector("#foodprint-calculator-iframe");
    if (!iframeEl) return;
    ...
    const schedulePost = () => {
      if (rafId) return;
      rafId = window.requestAnimationFrame(postState);
    };
    ...
    window.addEventListener("message", (event) => {
      if (event.data && event.data.type === "EMBED_CHILD_READY") {
        schedulePost();
      }
    });
...
const EXPLANATIONS = {
  ...
  8: "Milk, yogurt and some plant-based substitutes contain protein, calcium, and several vitamins. Dairy products' moderate environmental impacts are due to the fertilizer and other resources for growing feed grains and cows belching methane and excreting manure. Plant-based milks have lower impacts.",
  // Q9. Full-fat dairy products
  9: "Milk, yogurt and some plant-based substitutes contain protein, calcium, and several vitamins, but they also contain saturated fat and some cholesterol. The fat in these foods adds calories, and the abundant saturated fat may increase the risk of heart disease. Dairy products' moderate environmental impacts are due to belching of methane, need for feed grains, and excretion of manure, but plant-based milks have lower impacts.",
  // Q10. Chicken, turkey, eggs
...
function initScoreboardToggle() {
  ...
  const applyMode = () => {
    if (mql.matches) {
      toggle.hidden = false;
      // ALWAYS start collapsed in vertical mode
      setCollapsed(true);
    } else {
      toggle.hidden = true;
      setCollapsed(false);
  ...
  toggle.addEventListener("click", () => {
    ...
    const collapsed = !board.classList.contains("scoreboard--collapsed");
  ...
  const handleChange = () => {
    if (mql.matches) {
      toggle.hidden = false;
      // When switching into vertical mode, force collapsed every time
      setCollapsed(true);
    } else {
      toggle.hidden = true;
      setCollapsed(false);
...
function setupEmbeddedStickyScoreboard() {
  let embedded = false;
  try {
  ...
  const scoreboard = aside ? aside.querySelector(".scoreboard") : null;
  if (!layout || !aside || !scoreboard || !aside.parentNode) return;

  const verticalMql = window.matchMedia("(max-width: 1600px)");
  const spacer = document.createElement("div");
  spacer.className = "embedded-sticky-spacer";
  spacer.setAttribute("aria-hidden", "true");
  spacer.style.display = "none";
  aside.parentNode.insertBefore(spacer, aside);

  const state = {
    lastParentScrollY: 0,
    parentViewportH: window.innerHeight,
    frameTop: null,
    frameLeft: null,
  ...
  const getStickyTopPx = () => (verticalMql.matches ? 0 : 16);

  const setSpacer = (active) => {
    if (active) {
      const asideStyles = window.getComputedStyle(aside);
      spacer.style.display = "block";
      spacer.style.height = `${aside.offsetHeight}px`;
      spacer.style.gridColumn = asideStyles.gridColumn;
      spacer.style.justifySelf = asideStyles.justifySelf;
      spacer.style.order = asideStyles.order;
    } else {
      spacer.style.display = "none";
      spacer.style.height = "";
      spacer.style.gridColumn = "";
      spacer.style.justifySelf = "";
      spacer.style.order = "";
    }
  };

  const setFlowStyles = () => {
    aside.classList.remove("is-embedded-fixed");
  ...
  const getFrameOffset = () => {
    let top = typeof state.frameTop === "number" ? state.frameTop : 0;
    let left = typeof state.frameLeft === "number" ? state.frameLeft : 0;
    ...
    try {
      ...
      // frameElement may be inaccessible in some embedding contexts
    }

    return { top, left };
  };

  const applyMode = (nextMode, metrics) => {
    if (nextMode === "flow") {
      setFlowStyles();
      setSpacer(false);
      state.mode = "flow";
    ...
    if (nextMode === "fixed") {
      aside.classList.add("is-embedded-fixed");
      aside.style.position = "fixed";
      aside.style.top = `${metrics.stickyTopPx}px`;
      aside.style.left = `${state.cachedAsideRect.left}px`;
      aside.style.width = `${state.cachedAsideRect.width}px`;
    ...
    if (nextMode === "dock") {
      ...
      const layoutDocTop = state.lastParentScrollY + metrics.layoutViewportTop;
      const layoutDocBottom = state.lastParentScrollY + metrics.layoutViewportBottom;
      const maxAsideDocTop = layoutDocBottom - asideHeight;
      const dockTop = Math.max(0, maxAsideDocTop - layoutDocTop);

      aside.classList.remove("is-embedded-fixed");
      aside.style.position = "absolute";
      aside.style.top = `${dockTop}px`;
      aside.style.left = `${state.cachedAsideRect.leftInLayout}px`;
      aside.style.width = `${state.cachedAsideRect.width}px`;
      aside.style.zIndex = "30";
      state.mode = "dock";
    }
  ...
  const updatePosition = () => {
    state.rafId = 0;
    if (!state.hasParentScroll) return;

    const stickyTopPx = getStickyTopPx();
    const frameOffset = getFrameOffset();
    const layoutRect = layout.getBoundingClientRect();
    const anchorEl = spacer.style.display === "none" ? aside : spacer;
    const anchorRect = anchorEl.getBoundingClientRect();

    state.cachedAsideRect.left = anchorRect.left;
    state.cachedAsideRect.width = anchorRect.width;
    state.cachedAsideRect.leftInLayout = anchorRect.left - layoutRect.left;

    const layoutViewportTop = frameOffset.top + layoutRect.top;
    const layoutViewportBottom = frameOffset.top + layoutRect.bottom;
    const anchorViewportTop = frameOffset.top + anchorRect.top;
    ...
    const shouldPin = anchorViewportTop <= stickyTopPx;
    if (!shouldPin) {
      applyMode("flow", metrics);
      return;
    }

    const asideHeight = aside.offsetHeight;
    const fixedAsideDocTop = state.lastParentScrollY + stickyTopPx;
    const layoutDocBottom = state.lastParentScrollY + layoutViewportBottom;
    const maxAsideDocTop = layoutDocBottom - asideHeight;

    if (fixedAsideDocTop >= maxAsideDocTop) {
      applyMode("dock", metrics);
    } else {
      applyMode("fixed", metrics);
    }
  };
  ...
  window.addEventListener("message", (event) => {
    if (!event.data || event.data.type !== "PARENT_SCROLL") return;
    state.hasParentScroll = true;
    state.lastParentScrollY = Number(event.data.scrollY) || 0;
    state.parentViewportH = Number(event.data.viewportH) || window.innerHeight;
    if (typeof event.data.iframeTop === "number") {
      state.frameTop = event.data.iframeTop;
    }
    if (typeof event.data.iframeLeft === "number") {
      state.frameLeft = event.data.iframeLeft;
    }
    scheduleUpdate();
  });
  ...
  if (verticalMql.addEventListener) {
    verticalMql.addEventListener("change", onResize);
  } else {
    verticalMql.addListener(onResize);
  }
  ...
  try {
    window.parent.postMessage({ type: "EMBED_CHILD_READY" }, "*");
  } catch (_err) {
    // no-op
  }
}
